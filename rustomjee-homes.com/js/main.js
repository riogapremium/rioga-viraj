...
// Form Submit Operations
    ...
    // ?utm_source=Google&utm_medium=CPC&utm_campaign=Rustomjee+Multi+Projects+-+NRI&utm_term=hello

    var utmurl = "?utm_source="+$("#utm_source").attr("value")+"&utm_medium="+$("#utm_medium").attr("value")+"&utm_campaign="+$("#utm_campaign").attr("value")+"&utm_term="+$("#utm_term").attr("value");
    // alert(utmurl);
    // alert(typeofDataForm)
    
    $(".passUTM").each(function(){
        var temputm = $(this).attr("href")
        $(this).attr("href",temputm + utmurl)
    })
    // console.log(dataShow)

    if(typeofDataForm == "Check Price" || typeofDataForm == "View Floor Plan" || typeofDataForm == "Download Plans"){
        $(".loadMessage").html("Request Initiated, Will get back to you soon")
    }else if(typeofDataForm == "Download Brochure"){
        $(".loadMessage").html("Request Initiated, Brochure will be shared soon!")

    }else if(typeofDataForm == "Schedule Virtual Tour" || typeofDataForm == "Enquire Now" || typeofDataForm == "Know More" || typeofDataForm == "Play Video"){
...
// Aminities Tabs Start
    ...
    $(window).scroll(function(){
        ...
        if($("#projects").offset().top - $(window).scrollTop() < 0){
...
// $(".newfiltersectionsinner").clickToggle(function() {
//     $(".selectfilterdrop").hide()
//     $(this).find(".selectfilterdrop").show();
...
$(".newfiltersectionsinner").click(function () {
    $(".selectfilterdrop").fadeOut();
    $(this).find(".selectfilterdrop").fadeIn();
  });
  ...
  $(".filterlist").click(function (e) {
    ...
    $(".homesearchbtn").attr($(this).attr("data-filter"), $(this).html());
  ...
  $(".homesearchbtn").click(function () {
    ...
    var temploc = $(this).attr("data-loc");
    var tempstatus = $(this).attr("data-status");
    var temptype = $(this).attr("data-type");
    var tempprice = $(this).attr("data-price");
  
    var flag = 0;
  
    // Extract number from type if not "All"
    temptype = temptype === "All" ? "All" : temptype.match(/\d+/)[0];
  
    $(".OuterProj").hide();
  
    $(".OuterProj").each(function () {
      // Split attributes into arrays (trimmed)
      const locArr = $(this).attr("data-loc").split(",").map((v) => v.trim());
      const statusArr = $(this).attr("data-status").split(",").map((v) => v.trim());
      const typeArr = $(this).attr("data-type").split(",").map((v) => v.trim());
      const priceArr = $(this).attr("data-price").split(",").map((v) => v.trim());
  
     // Location match
    const locMatch = temploc === "All" || locArr.includes(temploc);

    // Status match
    const statusMatch = tempstatus === "All" || statusArr.includes(tempstatus);

    // Type match
    let selectedType = temptype === "All" ? "All" : parseInt(temptype);
    const projectTypes = typeArr.map(t => parseInt(t));
    const typeMatch = (selectedType === "All") || projectTypes.includes(selectedType);

    // Price match
    const priceMatch = tempprice === "All" || priceArr.includes(tempprice);

    // Show project if all match
    if (locMatch && statusMatch && typeMatch && priceMatch) {
        $(this).fadeIn(200);
        flag = 1;
    }
    });
    ...
    // Button animation
    $(this).css({ background: "#7a5d20", transform: "scale(1.02)" });
    setTimeout(() => {
      $(this).css({ background: "#b98e33", transform: "scale(1)" });
    }, 200);
...
//     var clickfilter = $(this).find(".selectfilterdrop")
...
$(".unitOptButton").click(function(){
   
});

$(".optionsBodyOuter").each(function(){
    $(this).parent().attr("data-bodyheight",$(this).height())
})

$('.unitOptButton').clickToggle(function() {
    $("img", this).css("rotate", "0deg")
    $(this).next().css("height",$(this).next().attr("data-bodyheight")+"px");
    $(".changeTextUnit", this).html("Hide Unit Options")
    
}, function() {
    $("img", this).css("rotate", "180deg")
    $(".changeTextUnit", this).html("Show Unit Options")
...
$(".locBtn").click(function(){
    ...
    var locationdisplay = $(this).attr("data-loctarget")
    $(".OuterProj").css("display","none")
    $("."+locationdisplay).css("display","flex")

    if( locationdisplay == 'All'){
        $(".OuterProj").css("display","flex")
    }
})
...
// Pop Up Close Code
    ...
    $(".popupBack, .closeGallery, .closeform").click(function(){
        ...
        if(flag=1){
            ...
            $("#project_name").attr("value","Godrej Properties")
        }
    ...
    $(".formPopTrigger").click(function(){
        ...
        if(formtypeData == "Schedule Virtual Tour"){
            $("#sitevisitform").show()
        }
        ...
        if(formtypeData == "Check Price" || formtypeData == "View Floor Plan" ){
            ...
            projname = $(this).attr("data-projname")
        ...
        if(formtypeData == "The 1% Plan"){
            formtype = "Get Offer Details"
            formtypeData = "offer"
            projname = "Godrej Properties"
        }
        ...
        if(formtypeData == "Enquire Now"){
            
            formtype = "Enquire Now"
            projname = $(this).attr("data-projname")
            // formtypeData = "Enquire Now"
        }

        if(formtypeData == "Know More"){
...
// setTimeout(()=>{
//     $(".popupOffer").css("display","flex")
    
// },1500)

// $(".closeOfferInner2").click(function(){
//     $(".popupOffer").fadeOut()
// })
...
form.addEventListener('submit', (e) => {
    ...
    // .then(responseText => {
        ...
        let link = 'bit.ly/rstmjee';
        let website = 'https://rustomjee-homes.com/';
        let salesnumber = '919930077799';
        let salesemail = 'amitr@riogapremium.com';
        let salesname = 'Amit R';
    ...
    .then(emailResponse => {
        ...
        window.location.href = "https://rustomjee-homes.com?showData=true&type=" + formtypeData + "&plan=" + plantrigger;
    })
    .catch(error => {
